...
function loadState() {
  try {
    ...
    if (raw) {
      ...
      return { ...defaultState(), ...parsed, config: { ...defaultConfig, ...(parsed.config || {}) } };
    }
...
export default function App() {
  ...
  const addFav = (item) => setState((s) => {
    ...
    const nuevo = { id: Date.now() + "_" + Math.random().toString(36).slice(2, 7), nombre: nom, kcal: +item.kcal || 0, prot: +item.prot || 0, grasas: +item.grasas || 0, carbos: +item.carbos || 0 };
...
function Comida({ state, cfg, sel, setSel, setDay, setSub, setConfig, aguaVasosObj, addFav, delFav }) {
  ...
  const [nuevo, setNuevo] = useState({ comida: "", nombre: "", kcal: "", prot: "", grasas: "", carbos: "" });
  const [scanMeal, setScanMeal] = useState(null);
  const [favEdit, setFavEdit] = useState(false);

  const MEALS = ["desayuno", "comida", "cena", "snack"];
  const sumDay = (comidas, key) => MEALS.reduce((s, m) => s + (comidas[m] || []).reduce((a, x) => a + (+x[key] || 0), 0), 0);
  const totalKcal = sumDay(d.comidas, "kcal");
  const totalProt = sumDay(d.comidas, "prot");
  const totalGrasas = sumDay(d.comidas, "grasas");
  const totalCarbos = sumDay(d.comidas, "carbos");
  const recalc = (comidas) => ({ kcal: sumDay(comidas, "kcal"), proteina: sumDay(comidas, "prot"), grasas: sumDay(comidas, "grasas"), carbos: sumDay(comidas, "carbos") });

  // añade un alimento a una comida y recalcula los totales del día desde cero
  const pushFood = (meal, item) => {
    const nom = (item.nombre || "").trim();
    if (!nom) return;
    const arr = [...(d.comidas[meal] || []), { nombre: nom, kcal: +item.kcal || 0, prot: +item.prot || 0, grasas: +item.grasas || 0, carbos: +item.carbos || 0 }];
    setSub(sel, "comidas", { [meal]: arr });
    setDay(sel, recalc({ ...d.comidas, [meal]: arr }));
  };

  const addComida = (meal) => {
    if (nuevo.comida !== meal) return;
    pushFood(meal, { nombre: nuevo.nombre, kcal: nuevo.kcal, prot: nuevo.prot, grasas: nuevo.grasas, carbos: nuevo.carbos });
    setNuevo({ comida: "", nombre: "", kcal: "", prot: "", grasas: "", carbos: "" });
  };
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
        ...
        {modo === "detallado" && <MacroCard label="Grasas" cur={totalGrasas} obj={cfg.grasas} suffix="g" color={C.gold} soft={"#F1E7CC"} invert />}
        {modo === "detallado" && <MacroCard label="Hidratos" cur={totalCarbos} obj={cfg.carbos} suffix="g" color={C.blue} soft={C.blueSoft} invert />}
      </div>
            ...
            <Section key={meal} title={meal[0].toUpperCase() + meal.slice(1)}>
              <div style={{ display: "grid", gap: 7 }}>
                {(d.comidas[meal] || []).map((x, i) => (
                  <div key={i} style={{ ...card(), padding: "10px 13px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div><div style={{ fontWeight: 600, fontSize: 14 }}>{x.nombre}</div><div style={{ fontSize: 12, color: C.muted }}>{x.kcal} kcal · P{x.prot || 0} · G{x.grasas || 0} · H{x.carbos || 0}</div></div>
              ...
              {nuevo.comida === meal ? (
                <div style={{ ...card(), padding: 12, marginTop: 8, display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <input value={nuevo.grasas} onChange={(e) => setNuevo({ ...nuevo, grasas: e.target.value })} placeholder="grasas (g)" inputMode="numeric" style={{ ...inp, flex: 1 }} />
                    <input value={nuevo.carbos} onChange={(e) => setNuevo({ ...nuevo, carbos: e.target.value })} placeholder="hidratos (g)" inputMode="numeric" style={{ ...inp, flex: 1 }} />
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => { if (nuevo.nombre.trim()) addFav({ nombre: nuevo.nombre, kcal: nuevo.kcal, prot: nuevo.prot, grasas: nuevo.grasas, carbos: nuevo.carbos }); }} title="Guardar como favorito" style={{ background: C.greenSoft, color: C.greenDeep, border: "none", borderRadius: 10, padding: "0 13px", fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}><Star size={15} /> Guardar</button>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <button onClick={() => setNuevo({ comida: meal, nombre: "", kcal: "", prot: "", grasas: "", carbos: "" })} style={{ flex: 1, padding: 11, borderRadius: 12, border: `1.5px dashed ${C.line}`, background: "transparent", color: C.green, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><Plus size={16} /> Añadir alimento</button>
...
function Scanner({ onClose, onResult }) {
  ...
  const lookup = async (code) => {
    ...
    try {
      ...
      const grasas100 = n["fat_100g"];
      const carbos100 = n["carbohydrates_100g"];
      ...
      setProd({
        ...
        grasas100: grasas100 != null ? Math.round(grasas100 * 10) / 10 : null,
        carbos100: carbos100 != null ? Math.round(carbos100 * 10) / 10 : null,
      });
      setBusy(false);
    } catch (e) {
  ...
  const confirmar = () => {
    if (!prod) return;
    const g = +gramos || 100;
    const calc = (per100) => (per100 != null ? Math.round(per100 * g / 100 * 10) / 10 : 0);
    onResult({
      nombre: `${prod.nombre} · ${g} g`,
      kcal: prod.kcal100 != null ? Math.round(prod.kcal100 * g / 100) : 0,
      prot: calc(prod.prot100),
      grasas: calc(prod.grasas100),
      carbos: calc(prod.carbos100),
    });
    close();
  };
            ...
            {!busy && prod && (
              <div style={{ display: "grid", gap: 14 }}>
                <div style={{ ...card(), padding: 14 }}>
                  ...
                  <div style={{ fontSize: 12.5, color: C.muted }}>Código {ean} · por 100 g: {prod.kcal100 != null ? `${prod.kcal100} kcal` : "kcal n/d"} · P {prod.prot100 != null ? prod.prot100 : "n/d"} · G {prod.grasas100 != null ? prod.grasas100 : "n/d"} · H {prod.carbos100 != null ? prod.carbos100 : "n/d"}</div>
                </div>
                <div>
                  ...
                  <div style={{ fontSize: 13, color: C.ink2, marginTop: 10 }}>
                    Se añadirá: <b>{prod.kcal100 != null ? Math.round(prod.kcal100 * (+gramos || 0) / 100) : 0} kcal</b> · P <b>{prod.prot100 != null ? Math.round(prod.prot100 * (+gramos || 0) / 100 * 10) / 10 : 0}</b> · G <b>{prod.grasas100 != null ? Math.round(prod.grasas100 * (+gramos || 0) / 100 * 10) / 10 : 0}</b> · H <b>{prod.carbos100 != null ? Math.round(prod.carbos100 * (+gramos || 0) / 100 * 10) / 10 : 0}</b> g
                  </div>
                </div>
...
function Ajustes({ cfg, setConfig, state, setState }) {
  ...
  const campos = [
    ...
    { k: "grasas", label: "Grasas objetivo (g)", step: 5 },
    { k: "carbos", label: "Hidratos objetivo (g)", step: 10 },
